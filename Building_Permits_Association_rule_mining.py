...
def loadDataSet():
    with open('./input/test2.csv','r',encoding="utf-8") as csvfile:
    ...
    if len(Item) >= 2:
        for element in itertools.combinations(list(Item), 1):
            if support_dic[Item] / float(support_dic[Item - frozenset(element)]) >= minConf:
                print(str([Item - frozenset(element)]) + "----->" + str(element), file = file)
                print (support_dic[Item] / float(support_dic[Item - frozenset(element)]), file = file)
